"""
r2_htf_overlay/harness_gate.py -- VERBATIM COPY of lab/harness.py (2026-09-18) plus ONE field:
`Cfg.entry_gate(now, side, entry_px) -> bool`, checked with the other modelable entry gates
(after `regime`). Used only for the PROTOCOL.md §1 appendix re-run check. Not for production.
...
class Cfg:
    ...
    # r2_htf_overlay addition: an extra entry gate, a pure function of (now, side, entry_px).
...
def _replay_body(module_name: str, mod, bars: dict, start, end, cfg: "Cfg") -> dict:
    ...
    for i in range(i0, i1):
        ...
        if regime_gate is not None and not regime_gate.admits(now):
            continue
        if cfg.entry_gate is not None and not cfg.entry_gate(now, sig.side, float(sig.entry_price)):
            continue
