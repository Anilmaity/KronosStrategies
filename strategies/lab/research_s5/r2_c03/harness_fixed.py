"""
harness_fixed.py -- r2_c03 copy of lab/harness.py with ONE change (closed-bar M5/M15 windows, see the FIX comment).
...
def _replay_body(module_name: str, mod, bars: dict, start, end, cfg: "Cfg") -> dict:
    ...
    for i in range(i0, i1):
        ...
        # r2_c03 FIX (2026-09-18): closed bars only. An M5/M15 bar enters the window once its
        # close <= this M1 bar's close (t1[i] + 1 min), i.e. open <= t1[i] - 4 min / - 14 min --
        # the runner's `complete` filter. The original included the bar in progress.
