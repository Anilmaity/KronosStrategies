"""
r2_s14/harness_closed.py -- COPY of lab/harness.py (2026-09-18) with ONE addition: Cfg.closed_frames.
When True the M5/M15 windows contain only bars CLOSED at the M1 close the runner reacts to
(start <= t1[i]-4min for M5, <= t1[i]-14min for M15), matching research_runner's complete-candle
feed. With False the behaviour is byte-identical to lab/harness.py (verified: reproduces the
incumbent arm exactly). Do not use for anything but this study.
...
_STRAT = _HERE.parents[2]   # lab/research_s5/r2_s14 -> strategies
...
class Cfg:
    ...
    # r2_s14 addition: hand get_signal only CLOSED M5/M15 bars (live fidelity).
    closed_frames: object = False     # True | False | "m5" | "m15"
    # r2_s14 addition (H5d): BUY only while newest CLOSED daily close > SMA(regime_sma),
    # SELL only while <= SMA. Needs the "1d" frame. Independent of `regime`.
...
def _replay_body(module_name: str, mod, bars: dict, start, end, cfg: "Cfg") -> dict:
    ...
    if cfg.regime_align:
        if "1d" not in bars:
            raise ValueError("cfg.regime_align needs the daily frame")
        align_gate = RegimeGate(bars["1d"], cfg.regime_sma, "above_sma20")
    regime_gate = None
    ...
    for i in range(i0, i1):
        ...
        # bar closed by the M1 close (t1[i] + 1 min): start <= t1[i] - (tf - 1 min).
        # closed_frames: True = both frames, "m5" / "m15" = that frame only (H0-ext), False = lab/harness.py
        cf = cfg.closed_frames
        if cf is True or cf == "m5":
            j5 = int(np.searchsorted(t5, t1[i] - np.timedelta64(4, "m"), side="right"))
        else:
            j5 = int(np.searchsorted(t5, t1[i], side="right"))
        if cf is True or cf == "m15":
        ...
        if align_gate is not None:
            above = align_gate.admits(now)          # False also while the SMA is undefined
            if (sig.side == "BUY") != above:
                continue
