"""
backtest_concepts.py
---------------------
Parallel backtest over concept_strategies (c01..c20). Mirrors
backtest_all_cache_mp.py but routes to the concept_strategies package so the
two strategy families don't collide in the same run.
"""
...
def _worker(args):
    ...
    from concept_strategies import STRATEGIES as _ALL
...
def main():
    ...
    tmp = tempfile.mkdtemp(prefix="kronos_concepts_")
    ...
    from concept_strategies import STRATEGIES
    strat_names = [s.NAME for s in STRATEGIES]

    # Optional CLI filter: --strategies c06 c07 ... (case-insensitive prefix match on NAME)
    if "--strategies" in sys.argv:
        idx = sys.argv.index("--strategies")
        wanted = [a.upper() for a in sys.argv[idx + 1:] if not a.startswith("--")]
        if wanted:
            strat_names = [n for n in strat_names if any(n.upper().startswith(w) or w in n.upper() for w in wanted)]

    if not strat_names:
        print("[ERR] no concept strategies registered")
        return
    n_workers = min(len(strat_names), cpu_count())
    print(f"Dispatching {len(strat_names)} concept strategies across {n_workers} workers: {strat_names}", flush=True)
    ...
    for name, trades in results:
        print(f"  {name:22s} {len(trades):,} trades", flush=True)
        all_trades.extend(trades)
    print(f"Total {len(all_trades):,} trades across all concept strategies in {time.time()-t2:.1f}s", flush=True)
    ...
    out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "results",
                       f"concept_strategies_{stamp}.csv")
