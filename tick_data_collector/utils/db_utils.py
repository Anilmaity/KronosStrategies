...
def connect_db() -> "psycopg2.extensions.connection":
    ...
    Prefers TIGERDATA_URL (cloud tick-store DSN) when set, falls back to
    DB_HOST/PORT/USER/PASSWORD vars for local development.
    """
    tigerdata_url = os.getenv("TIGERDATA_URL", "").strip()
    if tigerdata_url:
        try:
            conn = psycopg2.connect(tigerdata_url)
            print(f"[DB] Connected via TIGERDATA_URL")
            return conn
        except Exception as exc:
            print(f"[DB] FATAL – cannot connect via TIGERDATA_URL: {exc}")
            sys.exit(1)
